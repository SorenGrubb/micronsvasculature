...
**Radius caps and axis-aligned elongation** (`extract_pack_v6.py`, current version). Two rounds of feedback from Søren after actually looking at the render: some ellipsoids were still physically implausible (bigger than any real vessel), and — separately — he asked for elongated ellipsoids along each vessel's run instead of chains of touching spheres, both to look smoother and to cut annotation count. Two changes:

1. *Hard radius cap.* After smoothing, every radius is clamped to an anatomical bound Søren provided directly: no traced vessel exceeds 50 micron diameter (25,000 nm radius) anywhere in this dataset, and penetrating arterioles / ascending venules specifically cap at 25 micron diameter (12,500 nm radius). This replaces the earlier "smoothing damps but doesn't eliminate outliers" caveat — outliers are now hard-clamped, not just damped.
2. *Axis-aligned stretching.* Neuroglancer's ellipsoid annotation has no rotation/orientation field, so a true capsule oriented along a vessel's own (possibly diagonal) local direction isn't representable — only a stretch along a global x/y/z axis is. Where an MST edge runs close to one global axis (over 85% of its length along a single axis — true for roughly half of edges in these categories, consistent with vessels descending close to perpendicular to the cortical surface), the two rings' own ellipsoids are stretched along that axis until they overlap, with no new annotation needed. Edges that aren't well-aligned keep the earlier interpolated-filler-sphere approach unchanged. This cut counts meaningfully for the less-branchy categories (pial_arterioles 1482→1232, pial_venules 3721→2855, penetrating_arterioles 7975→6408) but not for ascending_venules, where the size headroom created by stretching was spent on a tighter (better-looking) interpolation multiplier instead (9013→9739 ellipsoids, but the gaps that remain are smaller, not larger). Final encoded sizes, measured against the actual packed JSON: pial_arterioles 0.17 MB, penetrating_arterioles 0.89 MB, ascending_venules 1.35 MB, pial_venules 0.39 MB — all comfortably under the tool's 1.95 MB warning threshold.

If a category's outline still looks unacceptably coarse, sized wrong, gappy, or the file is unusably large, tell the tool's maintainer which category and what's wrong — the RDP tolerance (capillaries), the endpoint-gap threshold or minimum-radius floor (ring/arc pairing, currently 200 nm and 50 nm), the MST edge cap / spacing multiplier / size budget (vessel-graph step, currently 20,000 nm / 2.0× / 1.7 MB), or the axis-alignment threshold / per-category radius cap (currently 0.85 and 25,000/12,500 nm) are the parameters to revisit.
