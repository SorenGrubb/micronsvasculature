...
Consecutive capsules along a capillary are then merged where it is safe to do so, which cut counts by a further ~25-30% (e.g. `venous_cap_order3` 13480 → 9761). The merge test is the part that matters: a merged capsule is *straight* while the traced centreline *curves* through it, so a merge judged only on its own endpoints — or on its members' centres — can look fine and still leave the real path outside. Measured, that naive version cost about 20 points of centreline coverage for the same reduction. Because capillary capsules are generated in order along the path, a candidate merge is just a consecutive slice, and the traced points it must cover are exactly those falling within its span; a merge is accepted only if every one of them ends up inside. Reduction therefore cannot quietly reintroduce gaps.

Verified against the traced centrelines: 85–97% of traced points fall inside some capsule, with interior sample points sitting a median 0.6–1.4 vessel radii from the centreline (i.e. roughly where the lumen wall should be). The lower end of that range is the 2nd/3rd-order venous categories, whose vessels are both the thinnest and the most tortuous — merging a straight capsule across a curve is exactly where coverage is hardest to keep. Without any merging these categories reach 97-98% coverage at ~35% more ellipsoids; the merging parameters (`CAP_MERGE_MAX`, `CAP_MERGE_PASSES`) are the knob if that trade should be reversed. Note that `venous_cap_order2` and `venous_cap_order3` previously had **half their chains dropped** by Morton-order subsampling to control size; that subsampling is gone, so every traced capillary is now present. Their higher ellipsoid counts buy back real vessels rather than added detail.
